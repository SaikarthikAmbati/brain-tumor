...
from tensorflow.keras.layers import Conv2D  # Conv2D is the updated name in tf.keras
...
def trainTumorDetectionModel():
    ...
    if os.path.exists('Model/model.json'):
        ...
        classifier.load_weights("Model/model_weights.h5")
        # No need to call _make_predict_function() here
    else:
        X_trains, X_tests, y_trains, y_tests = train_test_split(x_train, y_train, test_size = 0.2, random_state = 0)
        classifier = Sequential() 
        classifier.add(Conv2D(32, (3, 3), input_shape=(x_train.shape[1], x_train.shape[2], x_train.shape[3]), activation='relu'))
        classifier.add(MaxPooling2D(pool_size=(2, 2)))
